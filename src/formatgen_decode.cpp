...
uint32_t ChannelWidth(char const *name, uint64_t const v, uint32_t chan)
{
	...
	if (IsInDXTC(name, v))
	{
		...
		case TinyImageFormat_DXTC_TYPE_SRGB:
			return 8; // this is wrong but good enough imho
		case TinyImageFormat_DXTC_TYPE_SFLOAT:
		case TinyImageFormat_DXTC_TYPE_UFLOAT:
			return 16;
		}
	}
	if (IsInETC(name, v))
	{
		...
		switch (bits)
		{
		case TinyImageFormat_ETC_BITS_8:
			return 8;
		case TinyImageFormat_ETC_BITS_11:
			return 11;
		}
	}
	...
	for (int i = 0; i < TinyImageFormat_PACK_NUM_CHANNELS; ++i)
	{
		...
		if (swizzle >= 0)
		{
			...
			case 64:
				break;
			default:
				LOGERROR("unpacked format can only be 8, 16, 32, 64 bits wide per channel: bits encoded as %d",
						 chanBitWidth);
				continue;
			}
									 ...
									 TinyImageFormat_Pack_Special const special)
{
	...
	{
	case 8:
		sprintf(output, "%s\t\t\t\tuint8_t val = *((uint8_t const*)in->pixel);\n", output);
		sprintf(nextPixelBuffer, "\t\t\t\tin->pixel = (void const*)(((uint8_t const*)in->pixel) + 1);\n");
		break;
	case 16:
		sprintf(output, "%s\t\t\t\tuint16_t val = *((uint16_t const*)in->pixel);\n", output);
		sprintf(nextPixelBuffer, "\t\t\t\tin->pixel = (void const*)(((uint16_t const*)in->pixel) + 1);\n");
		break;
	case 32:
		sprintf(output, "%s\t\t\t\tuint32_t val = *((uint32_t const*)in->pixel);\n", output);
		sprintf(nextPixelBuffer, "\t\t\t\tin->pixel = (void const*)(((uint32_t const*)in->pixel) + 1);\n");
		break;
	case 64:
		sprintf(output, "%s\t\t\t\tuint64_t val = *((uint64_t const*)in->pixel);\n", output);
...
#define TinyImageFormat_MOD_MACRO(x, y)                   \
	{                                                     \
		bool okay = DecodeLogicalPixelsF(#x, y, output);  \
		if (okay)                                         \
		{                                                 \
...
#define TinyImageFormat_MOD_MACRO(x, y)                                            \
	{                                                                              \
		bool okay = DecodeLogicalPixelsD(#x, y, output);                           \
		if (okay)                                                                  \
		{                                                                          \
			sprintf(buffer, "\t\tcase %s: return true;\n", "TinyImageFormat_" #x); \
			VFile_Write(file, buffer, strlen(buffer));                             \
		}                                                                          \
	}
#define TinyImageFormat_END_MACRO    \
