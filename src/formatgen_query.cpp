...
#define TinyImageFormat_MOD_MACRO(x, y)                                                   \
	sprintf(buffer, "\t\tcase %s: return 0x%.16" PRIX64 ";\n", "TinyImageFormat_" #x, y); \
...
uint32_t SizeOfBlock(char const *name, uint64_t v, uint64_t dim)
{
	...
	if (IsInPVRTC(name, v))
	{
		...
		if (dim == 0)
		{
			...
			switch (bits)
			{
			case TinyImageFormat_PVRTC_BITS_2:
				return 8;
			case TinyImageFormat_PVRTC_BITS_4:
				return 4;
			}
		}
		else if (dim == 1)
	...
	if (IsInASTC(name, v))
	{
		...
		switch (size)
		{
		case TinyImageFormat_ASTC_SIZE_1:
			return 1;
		case TinyImageFormat_ASTC_SIZE_4:
			return 4;
		case TinyImageFormat_ASTC_SIZE_5:
			return 5;
		case TinyImageFormat_ASTC_SIZE_6:
			return 6;
		case TinyImageFormat_ASTC_SIZE_8:
			return 8;
		case TinyImageFormat_ASTC_SIZE_10:
			return 10;
		case TinyImageFormat_ASTC_SIZE_12:
			return 12;
		}
	}
	...
	if (IsInCLUT(name, v) && dim == 0)
	{
		...
		switch (bs)
		{
		case TinyImageFormat_CLUT_BLOCKSIZE_1:
			return 1;
		case TinyImageFormat_CLUT_BLOCKSIZE_2:
			return 2;
		case TinyImageFormat_CLUT_BLOCKSIZE_4:
			return 4;
		case TinyImageFormat_CLUT_BLOCKSIZE_8:
			return 8;
		}
	}
...
#define TinyImageFormat_MOD_MACRO(x, y)                                                  \
	{                                                                                    \
		uint32_t tmp = WidthOfBlock(#x, y) * HeightOfBlock(#x, y) * DepthOfBlock(#x, y); \
		if (tmp > maxPixelCountOfBlock)                                                  \
			maxPixelCountOfBlock = tmp;                                                  \
	};
...
uint32_t ChannelBitWidth(char const *name, uint64_t v, uint32_t chan)
{
	...
	if (IsInPacked(name, v))
	{
		...
		switch (bits)
		{
		case TinyImageFormat_PACK_BITS_0:
			return 0;
		case TinyImageFormat_PACK_BITS_1:
			return 1;
		case TinyImageFormat_PACK_BITS_2:
			return 2;
		case TinyImageFormat_PACK_BITS_3:
			return 3;
		case TinyImageFormat_PACK_BITS_4:
			return 4;
		case TinyImageFormat_PACK_BITS_5:
			return 5;
		case TinyImageFormat_PACK_BITS_6:
			return 6;
		case TinyImageFormat_PACK_BITS_7:
			return 7;
		case TinyImageFormat_PACK_BITS_8:
			return 8;
		case TinyImageFormat_PACK_BITS_9:
			return 9;
		case TinyImageFormat_PACK_BITS_10:
			return 10;
		case TinyImageFormat_PACK_BITS_11:
			return 11;
		case TinyImageFormat_PACK_BITS_12:
			return 12;
		case TinyImageFormat_PACK_BITS_16:
			return 16;
		case TinyImageFormat_PACK_BITS_24:
			return 24;
		case TinyImageFormat_PACK_BITS_32:
			return 32;
		case TinyImageFormat_PACK_BITS_64:
			return 64;
		default:
			ASSERT(false);
		}
	...
	if (IsInDepthStencil(name, v))
	{
		...
		switch (bits)
		{
		case TinyImageFormat_DEPTH_STENCIL_BITS_0:
			return 0;
		case TinyImageFormat_DEPTH_STENCIL_BITS_8:
			return 8;
		case TinyImageFormat_DEPTH_STENCIL_BITS_16:
			return 16;
		case TinyImageFormat_DEPTH_STENCIL_BITS_24:
			return 24;
		case TinyImageFormat_DEPTH_STENCIL_BITS_32:
			return 32;
		default:
			ASSERT(false);
		}
	...
	if (IsInCLUT(name, v))
	{
		...
		switch (bits)
		{
		case TinyImageFormat_CLUT_BITS_0:
			return 0;
		case TinyImageFormat_CLUT_BITS_1:
			return 1;
		case TinyImageFormat_CLUT_BITS_2:
			return 2;
		case TinyImageFormat_CLUT_BITS_4:
			return 4;
		case TinyImageFormat_CLUT_BITS_8:
			return 8;
		}
		ASSERT(false);
	}
...
static uint32_t BitSizeOfBlock(char const *name, uint64_t v)
{
	...
	if (IsInDepthStencil(name, v))
	{
		...
		switch (bits)
		{
		case TinyImageFormat_DEPTH_STENCIL_TOTAL_SIZE_8:
			return 8;
		case TinyImageFormat_DEPTH_STENCIL_TOTAL_SIZE_16:
			return 16;
		case TinyImageFormat_DEPTH_STENCIL_TOTAL_SIZE_32:
			return 32;
		case TinyImageFormat_DEPTH_STENCIL_TOTAL_SIZE_64:
			return 64;
		default:
			ASSERT(false);
		}
	...
	if (IsInDXTC(name, v))
	{
		...
		switch (bytes)
		{
		case TinyImageFormat_DXTC_BLOCKBYTES_8:
			return 8 * 8;
		case TinyImageFormat_DXTC_BLOCKBYTES_16:
			return 16 * 8;
		default:
			ASSERT(false);
		}
...
int8_t PhysicalChannelToLogical(char const *name, uint64_t const v, uint32_t chan)
{
	...
	if (IsInDepthStencil(name, v))
	{
		...
		switch (swiz)
		{
		case TinyImageFormat_DEPTH_STENCIL_SWIZZLE_D:
			return TinyImageFormat_LC_Depth;
		case TinyImageFormat_DEPTH_STENCIL_SWIZZLE_S:
			return TinyImageFormat_LC_Stencil;
		case TinyImageFormat_DEPTH_STENCIL_SWIZZLE_0:
			return TinyImageFormat_LC_0;
		default:
			ASSERT(false);
		}
	...
	if (IsInCLUT(name, v))
	{
		...
		switch (type)
		{
		case TinyImageFormat_CLUT_TYPE_NONE:
			return TinyImageFormat_LC_0;
		case TinyImageFormat_CLUT_TYPE_RGB:
			return TinyImageFormat_LC_Red;
		case TinyImageFormat_CLUT_TYPE_SINGLE:
			return TinyImageFormat_LC_Red;
...
static double Min(char const *name, uint64_t v, uint32_t const chan)
{
	...
	if (IsInPacked(name, v))
	{
		...
		{
			if (bits != TinyImageFormat_PACK_BITS_16)
				ASSERT(false);
			union
			{
				struct
...
double MaxActual(char const *name, uint64_t v, uint32_t const chan, bool ignoreNormFormats)
{
	...
	if (IsInPacked(name, v))
	{
		...
		if (type == TinyImageFormat_PACK_TYPE_UINT)
		{
			...
			case TinyImageFormat_PACK_BITS_64:
				return (double)UINT64_MAX; // lossy
			default:
				ASSERT(false);
			}
		}
		...
		if (type == TinyImageFormat_PACK_TYPE_UFLOAT)
		{
			...
			{
			case TinyImageFormat_PACK_BITS_9:
				return float(0x1FF << 7); // this is with the shared exponent
			case TinyImageFormat_PACK_BITS_10:
				return 65000.0;
			case TinyImageFormat_PACK_BITS_11:
				return 65500.0;
...
#define TinyImageFormat_START_MACRO VFile_Write(file, isPrefixF, strlen(isPrefixF));
#define TinyImageFormat_MOD_MACRO(x, y)                                                               \
	sprintf(buffer, "\t\tcase 0x%" PRIX64 ": return %s;\n", EmbeddedHash(#x), "TinyImageFormat_" #x); \
	VFile_Write(file, buffer, strlen(buffer));
#define TinyImageFormat_END_MACRO VFile_Write(file, switchPostfixF, strlen(switchPostfixF));
#include "formatgen.h"
}
...
#define TinyImageFormat_START_MACRO \
	{                               \
		VFile_Write(file, prefix, strlen(prefix));
#define TinyImageFormat_MOD_MACRO(x, y)                                                                                        \
	if (PhysicalChannelToLogical(#x, y, i) != i)                                                                               \
	{                                                                                                                          \
		sprintf(buffer, "\t\t\tcase %s: return %s;\n", "TinyImageFormat_" #x, lnames[PhysicalChannelToLogical(#x, y, i) + 2]); \
		VFile_Write(file, buffer, strlen(buffer));                                                                             \
	};
#define TinyImageFormat_END_MACRO                   \
	sprintf(buffer, switchPostfixF, lnames[i + 2]); \
	VFile_Write(file, buffer, strlen(buffer));      \
	}
#include "formatgen.h"
	}
...
#define TinyImageFormat_START_MACRO \
	{                               \
		uint32_t count = 0;
#define TinyImageFormat_MOD_MACRO(x, y)            \
	LogicalToPhysicalChannels(#x, y, &l2p[count]); \
	count += 4;
#define TinyImageFormat_END_MACRO }
#include "formatgen.h"
...
#define TinyImageFormat_MOD_MACRO(x, y)                                     \
	if (l2p[(count * 4) + i] != i)                                          \
	{                                                                       \
		WriteL2PPack(file, "TinyImageFormat_" #x, l2p[(count * 4) + i], y); \
	}                                                                       \
	count++;
#define TinyImageFormat_END_MACRO                   \
	sprintf(buffer, switchPostfixF, pnames[i + 2]); \
	VFile_Write(file, buffer, strlen(buffer));      \
	}
#include "formatgen.h"
	}
